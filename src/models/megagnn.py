...
class DiscreteEncoder(nn.Module):
    def __init__(self, hidden_channels, max_num_features=10, max_num_values=500):  # 10
        ...
        self.embeddings = nn.ModuleList(
            [
                nn.Embedding(max_num_values, hidden_channels)
...
class TransformerAgg(nn.Module):
    ...
    def forward(self, x, index, timestamps_):
        ...
        sorted_dense_edge_feats, sorted_mask = self.sort_wrt_time(
            dense_edge_feats, mask
        )

        s_post = time.time()
        sorted_dense_edge_feats = self.pos_enc(
            sorted_dense_edge_feats.permute(1, 0, 2)
        ).permute(1, 0, 2)
        sorted_dense_edge_feats = self.trans_enc(
            sorted_dense_edge_feats, src_key_padding_mask=~sorted_mask
        )
        sorted_dense_edge_feats[
            ~sorted_mask.unsqueeze(-1).expand(-1, -1, sorted_dense_edge_feats.shape[-1])
        ] = 0
        s_res = time.time()

        logging.debug(
            f"Preprocessing Time Transformer: {s_post - s_pre} | Transformer forward pass: {s_res - s_post}"
        )
    ...
    def sort_wrt_time(self, matt, mask):
        ...
        sorted_matt = torch.gather(
            matt, 1, sort_indices.unsqueeze(-1).expand(-1, -1, matt.shape[-1])
        )
...
class GRUAgg(nn.Module):
    def __init__(self, d_model=66):
        ...
        self.gru = nn.GRU(d_model, hidden_size=d_model, num_layers=2, batch_first=True)
...
class GinAgg(nn.Module):
    def __init__(self, n_hidden):
        ...
        self.nn = nn.Sequential(
            nn.Linear(n_hidden, n_hidden), nn.ReLU(), nn.Linear(n_hidden, n_hidden)
        )
...
class MLP(nn.Module):
    def __init__(
        ...
        self.norms = nn.ModuleList(
            [
                (
                    nn.BatchNorm1d(n_hid if i < nlayer - 1 else nout)
                    if not LN
                    else nn.LayerNorm(n_hid) if with_norm else Identity()
                )
                for i in range(nlayer)
            ]
        )
...
class AdammAgg(nn.Module):
    def __init__(self, n_hidden):
        ...
        self.edge_transform = MLP(
            n_hidden, n_hidden, nlayer=1, with_final_activation=False
        )
...
class MultiEdgeAggModule(nn.Module):
    ...
    def forward(self, edge_index, edge_attr, simp_edge_batch, times):
        ...
        new_edge_index = (
            scatter(edge_index, inverse_indices, dim=1, reduce="mean")
            if self.agg_type is not None
            else edge_index
        )

        if (self.agg_type == "transformer") or (self.agg_type == "gru"):
            new_edge_attr = self.agg(
                x=edge_attr, index=inverse_indices, timestamps_=times
            )
        else:
...
class MEGAGnnHelper(torch.nn.Module):
    def __init__(
        self,
        num_gnn_layers,
        n_hidden=100,
        ...
        for _ in range(self.num_gnn_layers):
            ...
            if self.flatten_edges:
                edge_agg = MultiEdgeAggModule(
                    n_hidden, agg_type=args.edge_agg_type, index=index_
                )
    ...
    def forward(self, x, edge_index, edge_attr, simp_edge_batch=None):
        ...
        if self.edge_agg_type == "adamm":
            ...
            edge_index, edge_attr, _ = self.edge_agg(
                edge_index, edge_attr, simp_edge_batch, times
            )
            src, dst = edge_index
            ...
            for i in range(self.num_gnn_layers):
                if self.flatten_edges:
                    n_edge_index, n_edge_attr, inverse_indices = self.edge_aggrs[i](
                        edge_index, edge_attr, simp_edge_batch, times
                    )
                    x = (
                        x
                        + F.relu(
                            self.batch_norms[i](
                                self.convs[i](x, n_edge_index, n_edge_attr)
                            )
                        )
                    ) / 2
                    if self.edge_updates:
                        remapped_edge_attr = torch.index_select(
                            n_edge_attr, 0, inverse_indices
                        )  # artificall node attributes
                        edge_attr = (
                            edge_attr
                            + self.emlps[i](
                                torch.cat(
                                    [x[src], remapped_edge_attr, edge_attr], dim=-1
                                )
                            )
                            / 2
                        )
