...
class GIN(nn.Module):
    ...
    def __init__(
        self, n_layers: int, in_dims: int, hidden_dims: int, out_dims: int, create_mlp: Callable[[int, int], MLP], config: Any,
...
class GINSampleAggregator(nn.Module):
    ...
    def __init__(
        self, n_layers: int, in_dims: int, hidden_dims: int, out_dims: int, create_mlp: Callable[[int, int], MLP], bn: bool, config,
    ) -> None:
        super().__init__()
        self.gin = GIN(n_layers, in_dims, hidden_dims, out_dims, create_mlp, config, bn, laplacian=None)
...
def GetSampleAggregator(cfg: Any, create_mlp: Callable[[int, int], MLP], config, device):
    return GINSampleAggregator(cfg.n_sample_aggr_layers, cfg.pearl_mlp_out, cfg.sample_aggr_hidden_dims, cfg.pe_dims,
                                        create_mlp, cfg.batch_norm, config)
