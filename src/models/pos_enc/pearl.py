...
def create_mlp(in_dims: int, out_dims: int, cfg: Any, use_bias: bool = True) -> MLP:
    return MLP(
        cfg.n_mlp_layers, in_dims, cfg.mlp_hidden_dims, out_dims, cfg.mlp_use_bn,
        cfg.mlp_activation, cfg.mlp_dropout_prob, use_bias=use_bias
        )


def get_PEARL_wrapper(config):
    sample_aggr = GetSampleAggregator(config, create_mlp, config, config.device)
    pearl_encoder = PEARLPositionalEncoder(sample_aggr=sample_aggr, k = config.pearl_k, pearl_act = config.pearl_act, 
...
class PEARLWrapper(nn.Module):
    ...
    def forward(self, data):
        def _get_lap(data):
            ...
            n = data.num_nodes
            L_edge_index, L_values = get_laplacian(data.edge_index, normalization="sym", num_nodes=n)   # [2, X], [X]
